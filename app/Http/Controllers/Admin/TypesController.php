...
class TypesController extends Controller
{
    ...
    public function index(){
        ...
        //$data = DB::table("types")->select('types.*','concat(path,id) as p')->orderby('p')->paginate(3);
        $data = DB::table("types")->select(DB::raw("types.*,concat(path,id) as p  "))->orderby("p")->paginate(6);
        // echo "<pre>";
        // print_r($data);

        $tot = \DB::table('types')->count();
        //$data = \DB::table('types')->paginate(3);
        return view("admin.types.index")->with('data',$data)->with('tot',$tot);
    }
